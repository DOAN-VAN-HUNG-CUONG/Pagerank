...
social graphs; we report node counts
$N\in\{10^3,5\!\times\!10^3,10^4,5\!\times\!10^4\}$ (up to $1.7\times10^5$ edges).
...
4-vCPU containerized Linux environment (Ubuntu~22.04, x86-64, CPython~3.10,
3.8~GiB RAM). To quantify run-to-run variation, every (framework, size)
configuration was executed \emph{three times} and we report the mean and sample
standard deviation; the lone exception is the genuine \texttt{mrjob} MapReduce
engine at $N=10^4$, run once because of its high per-iteration cost. The harness
...
wall-clock time and peak memory for the four frameworks that run on a single node
without a cluster: the pure-Python engine, the \texttt{mrjob} local (core)
engine, the genuine single-pass \texttt{mrjob} MapReduce job, and the Hadoop
Streaming local pipeline. The two in-memory engines are fastest at every size and
scale near-linearly in $|E|$; the \texttt{mrjob} local engine's overhead is a
constant of its runner that is amortized as the graph grows---from $1.7\times$ at
$N=10^3$ to $\approx\!1.0\times$ at $N=5\times10^4$, where its $5.32$\,s is within
$2\%$ of the engine's $5.24$\,s. Hadoop Streaming is $8\times$--$14\times$ slower
because it spawns operating-system processes and re-serializes the entire state
through the \texttt{map$\,|\,$sort$\,|\,$reduce} pipeline on every iteration. The
genuine \texttt{mrjob} MapReduce job is slower still, $44\times$--$49\times$,
because it launches a fresh MapReduce job for each of the 30 iterations; this
isolates pure \emph{orchestration} overhead from the $O(|E|)$ computation, which
is identical to the engine's. Conversely, Streaming's peak memory is the lowest
($9$--$14$\,MB), since it streams records rather than holding the graph in a
single address space, whereas the in-memory engines grow with the graph (up to
$82$--$93$\,MB at $N=5\times10^4$)---a concrete illustration of the classic
time--memory trade-off between in-memory and streaming execution.

\begin{table*}[t]
\centering
\caption{Single-node performance, 30 iterations, Barab\'asi--Albert graphs,
measured in a 4-vCPU Linux container. Each cell is \emph{mean wall-clock time
(s)\,$\pm$\,std\,/\,peak memory (MB)} over three runs. The \texttt{mrjob}
MapReduce engine at $N{=}10^4$ was run once owing to its cost; ``---'' marks a
configuration not run on a single node.}
\label{tab:perf}
\begin{tabular}{@{}lcccc@{}}
\toprule
Framework & $N{=}10^3$ & $N{=}5\!\times\!10^3$ & $N{=}10^4$ & $N{=}5\!\times\!10^4$ \\
\midrule
Core (Python)      & 0.10\,$\pm$\,0.01 / 13.4 & 0.30\,$\pm$\,0.01 / 18.4 & 0.65\,$\pm$\,0.02 / 24.6 & 5.24\,$\pm$\,0.07 / 82.2 \\
mrjob (core)       & 0.17\,$\pm$\,0.01 / 24.2 & 0.44\,$\pm$\,0.12 / 29.3 & 0.71\,$\pm$\,0.02 / 35.4 & 5.32\,$\pm$\,0.02 / 92.7 \\
mrjob (MapReduce)  & 4.89\,$\pm$\,0.02 / 29.7 & 13.26\,$\pm$\,0.39 / 39.9 & 28.32 / 52.0 & --- \\
Hadoop Streaming   & 1.36\,$\pm$\,0.01 / \phantom{0}9.5 & 2.98\,$\pm$\,0.03 / 11.7 & 4.98\,$\pm$\,0.01 / 13.8 & --- \\
\bottomrule
\end{tabular}
\end{table*}
...
$|E|$; the single-pass Streaming and \texttt{mrjob} MapReduce pipelines carry
large per-iteration constants.}
\label{fig:runtime}
\end{figure}
...
to $5\times10^4$ nodes) the graph fits comfortably in memory, so the distributed
...
its $44\times$--$49\times$ slowdown. Hadoop Streaming similarly pays
...
sizes. In-memory Spark is designed to
